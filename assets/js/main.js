...
document.querySelectorAll('.experience-toggle').forEach(button => {
  button.addEventListener('click', () => {
    ...
    if (label) label.textContent = wasOpen ? 'Ver experiencia' : 'Cerrar detalle';
    if (icon) {
      icon.src = wasOpen
        ? 'imagenes/iconos/pagina%20experiencias/keyboard_arrow_down_24dp_2568AC_FILL0_wght500_GRAD0_opsz24.svg'
        : 'imagenes/iconos/pagina%20experiencias/keyboard_arrow_up_24dp_2568AC_FILL0_wght500_GRAD0_opsz24.svg';
    }
    detail.hidden = wasOpen;

    if (!wasOpen) {
      detail.querySelector('[tabindex="-1"]')?.focus();
      detail.querySelector('[data-gallery]')?.dispatchEvent(new Event('gallery:resize'));
    }
  });
});
...
const lightboxCounter = galleryLightbox?.querySelector('.gallery-lightbox__counter');
...
document.querySelectorAll('[data-gallery]').forEach(gallery => {
  ...
  const visibleItems = () => {
    ...
    return Math.max(1, Math.round((viewport.clientWidth + gap) / (itemWidth + gap)));
  };
  ...
  items.forEach((item, index) => {
    item.addEventListener('click', () => {
      ...
      lightboxOpener = item;
      const experienceTitle = gallery.closest('.experience')?.querySelector('.experience-main h2')?.textContent?.trim();
      if (lightboxTitle) lightboxTitle.textContent = experienceTitle || 'Galería de la experiencia';
      updateLightbox();
...
if (galleryLightbox) {
  ...
  galleryLightbox.querySelector('.gallery-lightbox__arrow--prev')?.addEventListener('click', () => moveLightbox(-1));
  galleryLightbox.querySelector('.gallery-lightbox__arrow--next')?.addEventListener('click', () => moveLightbox(1));
...
if (contactForm) {
  ...
  const showValidationMessage = () => {
    if (!contactForm.checkValidity() && status) {
      status.textContent = 'Revisa los campos obligatorios indicados antes de enviar.';
    }
  };

  contactForm.querySelector('[type="submit"]')?.addEventListener('click', showValidationMessage);
  contactForm.addEventListener('invalid', showValidationMessage, { capture: true });
  ...
  contactForm.addEventListener('submit', event => {
    ...
    if (status) {
      status.textContent = 'Gracias. Tu consulta quedó preparada; conectaremos el envío cuando exista un backend.';
    }
    contactForm.reset();
  });
}
